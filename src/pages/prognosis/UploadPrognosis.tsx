...
const UploadPrognosis: React.FC = () => {
  ...
  const handleSubmit = async () => {
    if (!geneFile) { setError("Please upload a gene expression file."); return; }
    setError(""); setLoading(true);
    const fd = new FormData();
    fd.append("gene_file", geneFile);
    try {
      const res    = await fetch(`${baseUrl}/predict/prognosis`, { method: "POST", body: fd });
      const result = await res.json();
      navigate("/result/prognosis", { state: { predictionResult: result } });
    } catch {
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Prognosis — Step 2 of 3</p>
              </div>
            </div>
            ...
            <div
              ...
              <label htmlFor="geneFile" className="cursor-pointer block">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-xs text-slate-400">Gene expression file</p>
                </div>
              </label>
              {geneFile && (
